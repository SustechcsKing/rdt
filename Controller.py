...
class Controller:
    def __init__(self, socket, data: bytes):
        ...
        self.sender = Sender(self.socket, self.send_buf, self.ack_list, self.ack_recv, self.window_size)
    ...
    #     while self.sender.window[0] < self.window_size[0] and self.tmp < len(self.send_list):
    #     # while self.sender.last_ack < self.window_size[0] and self.tmp < len(self.send_list):
    ...
    def send(self, to_send: bytes):  # 有问题
        # print("udihxfds", len(to_send))
        self.send_list.extend(datagram.segment(to_send, 256, pre=self.used_seq))
        # print("send_list_size_add:", (len(to_send) + 255) // 256)
        self.used_seq += (len(to_send) + 255) // 256
        while self.tmp < len(self.send_list):
            # print("SEQ", self.send_list[self.tmp].SEQ)
            self.send_buf.put((self.send_list[self.tmp].SEQ, self.send_list[self.tmp]))
            self.tmp += 1
        # print("to_send_buf:", self.send_buf.qsize())
        ...
        while True:
            if self.recv_buf.not_empty:
                ...
                if datagram.LEN != 0 or datagram.TYPE == 1:
                    self.ack_list.put(self.wanting)
    ...
    def recv_nonblocking(self):
        ...
        if self.recv_buf.not_empty:
            addr, datagram = self.recv_buf.get()
            # print("here wanting =",self.wanting,datagram.SEQ)
            # print("Recv Pack")
            # if datagram.LEN != 0 or datagram.ACK == 1:
            #     print("seq,data", datagram.ACK, datagram.SEQ, self.wanting, datagram.PAYLOAD)
            # if datagram.ACK == 1:
            #     print("sack", datagram.SACK)
            if datagram.ACK == 1:
                # print("received_ack", datagram.SACK)
                self.ack_pack(datagram.SACK)
            ...
            if datagram.SEQ == self.wanting:
                ...
                if not self.data_buf.empty():
                    ...
                    if flag == 1:
                        self.data_buf.put((seq, nxt_datagram))
                # print('put_ack', self.wanting - 1)
                self.ack_list.put(self.wanting - 1)
